...
with open(filepath,'r') as f:
    ...
    list = [next(f) for x in range(-1,day)]     # в лист записываются данные из f-файла
    for spl in list:
        scount = spl.split()
        spl1 = spl.replace('.',' ')
        spl2 = spl1.replace(':',' ')
        ...
        for i in range(0,5):
            time.append(float(stime[i]))    # время состоит из: год/месяц/день час:секунды --- 5 разделенных столбцов попадают в time
